...
public class PermissionManager {
    ...
    private final WeakReference<Object> mWeakRef;
                        ...
                        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
                            if (!shouldShowRequestPermissionRationale(permissions)) {
                                ...
                                if (isWeakNotNull() && mWeakRef.get() instanceof Activity) {
                                    ...
                                    ((Activity) mWeakRef.get()).startActivityForResult(intent, Constant.Permission.REQUEST_CODE);
                                } else if (isWeakNotNull() && mWeakRef.get() instanceof Fragment) {
                                    intent.setData(Uri.parse("package:" + context.getPackageName()));
                                    ((Fragment) mWeakRef.get()).startActivityForResult(intent, Constant.Permission.REQUEST_CODE);
                                }
                                return;
                            }
                        }
